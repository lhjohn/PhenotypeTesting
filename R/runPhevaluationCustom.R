...
#'   excludedCovariateConceptIds <<- pheValuatorAnalysisList[[1]]$createEvaluationCohortArgs$covariateSettings[[1]]$excludedCovariateConceptIds
...
#'   resultsFiles <- names(pheValuatorAnalysisList[[1]]$testPhenotypeAlgorithmArgs)
#'   if (length(resultsFiles) > 0) {
#'     createTask <- function(resultsFile) {
#'       args <- pheValuatorAnalysisList[[1]]$testPhenotypeAlgorithmArgs[[resultsFile]]
#'       args$connectionDetails <- connectionDetails
#'       args$cdmDatabaseSchema <- cdmDatabaseSchema
#'       args$cohortDatabaseSchema <- cohortDatabaseSchema
#'       args$cohortTable <- cohortTable
#'       args$outFolder <- file.path(outputFolder, referenceTable$evaluationCohortFolder[1])
#'       task <- list(args = args, fileName = file.path(outputFolder, paste0(resultsFile,".rds")))
#'       return(task)
#'     }
